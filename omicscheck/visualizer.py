...
# def plot_heatmap(df: pd.DataFrame, out_path: Path, top_n: int = 50):
#     """
#     Generate and save a clustered heatmap for top N variable genes.
#     - Normalizes gene expression using Z-score.
#     - Adds clustering for rows and columns.
#     """
#     if df.empty or df.shape[0] == 0 or df.shape[1] == 0:
...
#         scaler.fit_transform(filtered.T).T,
...
#         row_cluster=True
#     )
#     plt.savefig(out_path / "heatmap_clustered.png")
#     plt.close()
#
#
# def plot_heatmap_top_genes(df: pd.DataFrame, out_path: Path, top_n: int = 100):
#     """
#     Generate and save a clustered heatmap of the top N most variable genes (Z-score normalized).
#     """
#     top_genes = df.var(axis=1).sort_values(ascending=False).head(top_n).index
#     filtered = df.loc[top_genes]
#
#     scaler = StandardScaler()
#     zscore_data = pd.DataFrame(scaler.fit_transform(filtered.T).T,
#                                index=filtered.index,
#                                columns=filtered.columns)
#
#     cg = sns.clustermap(
...
#         cmap="vlag",
#         linewidths=0.2,
#         figsize=(max(12, zscore_data.shape[1] * 0.2), max(10, zscore_data.shape[0] * 0.2)),
#         xticklabels=True,
#         yticklabels=True,
#         col_cluster=True,
#         row_cluster=True,
...
#     )
#
#     cg.ax_heatmap.set_title(f"Clustered Heatmap of Top {top_n} Variable Genes", fontsize=14)
#     cg.ax_heatmap.set_xlabel("Samples", fontsize=12)
...
#
#     heatmap_path = out_path / f"heatmap_top_{top_n}_clustered.png"
